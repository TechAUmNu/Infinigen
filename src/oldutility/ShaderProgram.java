...
public class ShaderProgram {
	...
	/**
	 * Attach a Tesselation Control Shader to this program.
	 * 
	 * @param name
	 *            The file name of the vertex shader.
	 */
	public void attachTessControlShader(String name) {
		// Load the source
		String tessControlShaderSource = FileUtil.readFromFile(name);

		// Create the shader and set the source
		tessControlShaderID = glCreateShader(GL_TESS_CONTROL_SHADER);
		glShaderSource(tessControlShaderID, tessControlShaderSource);

		// Compile the shader
		glCompileShader(tessControlShaderID);

		// Check for errors
		if (glGetShaderi(tessControlShaderID, GL_COMPILE_STATUS) == GL_FALSE) {
			System.err.println("Unable to create tessControl shader:");

			// Print log
			System.err.println(glGetShaderInfoLog(tessControlShaderID, glGetShaderi(tessControlShaderID, GL_INFO_LOG_LENGTH)));
	 ...
	 *            The name of the uniform.
	 * @param matrix
	 *            FloatBuffer containing the matrix.
	 */
	...
	public void setUniform(String name, Vector3f cameraPos) {
		// Get the location of the uniform
		int location = glGetUniformLocation(programID, name);
		glUniform3d(location, cameraPos.x, cameraPos.y, cameraPos.z);
