...
public class ServerConnection implements Runnable, ActionListener {
	...
	public void run() {
		try {
			...
			Timer timer = new Timer(5, this); // With 5ms it will send the
												// message less than 5ms after
				...
				try {
					...
					if (!message.disconnect) {
						...
						if (message.sendClient) {
							// This will happen when the server asks for a
							// client info update.
							Message m = new Message(null, false, null, true, null, true, null);
							// Set the client username and teamID, the server
							// will fill in the IP address.
							m.SetClient(client);
							// Send the message
							sendMessage(m);
						}
						if (message.status != null) {
							...
							if (message.status.equals("done"))
								InterthreadHolder.getInstance().setLoading(false);
						}
						if (message.command != null) {
							ProcessInput(message);
						}
						if (message.disconnect) {
							// The disconnect flag on the message is set, so set
							// the disconnect flag to close the connection.
							disconnect = true;
						}
						// Clear the message so it is always null before
						// receiving one.
						message = null;
	...
	private void processUnsent() {
		...
		if (InterthreadHolder.getInstance().isServerReconciliation()) {
			// Server Reconciliation. Re-apply all the inputs not yet processed
			// by
			// the server.
	...
	// Process the client command that was received, we MUST process the
	// commands client side as well, if not then we will have serious jumpiness
	// problems!
	// It is important the processing code on the client always applies any
	// command from the server as it will be from other clients selecting
	// things.
	// Since we use TCP this "should" always be in order.
	// TODO: A check will probably be required to make sure that we are not
	// desynced, if we are then we request an update of the sprite positions
	// from the server.
	private void ProcessInput(Message m) {

		// Since we are applying all selects de-selects etc, we need to only
		// show ones relating to our client on the UI otherwise it will get VERY
		// confusing!
		switch (m.command.commandType) {
		case "Select":
			System.out.println("Client Processed Select order");
			for (UnitSprite s : InterthreadHolder.getInstance().getWorld().getPlayerSprites(InterthreadHolder.getInstance().getClient())) {
