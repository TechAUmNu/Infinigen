...
public class Unit {
	...
	private void createBaseCube(PhysicsProcessor processor) {
		...
		// Generic6DofConstraint binding =
		// BindEntities(base,jointTest,0.0f,2.3f,0.0f);
		entities.add(base);
	...
	public Generic6DofConstraint BindEntities(PhysicsEntity entity1, PhysicsEntity entity2, float x, float y, float z) {
		...
		processor.addConstraint(binding, true);
		return binding;
	}
	...
	public boolean spaceOcupied(float x, float y, float z) { // check if a space
																// is ocupied
	...
	public Unit Clone(float x, float y, float z) {
		...
		for (PhysicsEntity i : entities) {
			...
			// newBox.getPosition().y + " " + newBox.getPosition().z);
